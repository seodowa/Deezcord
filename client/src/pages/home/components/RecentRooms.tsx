  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between px-2">
        <h3 className="text-xl font-extrabold text-slate-900 dark:text-slate-50 tracking-tight">Recent Rooms</h3>
      ...
      <div className="relative group/carousel">
        ...
        {showRightArrow && recentRooms.length > 0 && (
          <button 
            ...
            className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-20 w-10 h-10 rounded-full bg-white dark:bg-slate-800 shadow-xl border border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-600 dark:text-slate-300 hover:scale-110 active:scale-95 transition-all opacity-0 group-hover/carousel:opacity-100"
          >
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
        ...
        <div 
          ref={scrollRef}
          className="flex overflow-x-auto gap-5 pb-8 pt-4 -mx-2 px-2 scrollbar-none snap-x snap-mandatory relative"
        >
          {isLoading ? (
            [1,2,3,4,5].map(i => (
              <div key={i} className="w-44 h-44 shrink-0 rounded-[2.5rem] bg-white/40 dark:bg-slate-800/40 animate-pulse border border-slate-200/50 dark:border-white/10"></div>
            ))
            ...
            <div className="flex-1 min-h-[176px] rounded-[2.5rem] bg-white/40 dark:bg-slate-800/40 backdrop-blur-xl border border-dashed border-slate-300 dark:border-slate-700 text-center flex flex-col items-center justify-center p-8">
